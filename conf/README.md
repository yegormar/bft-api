...
- **report_profile_system_prompt.txt** – System prompt for report profile summary (LLM synthesis from full session payload). Asks for three sections: personality-at-a-glance (horoscope-style), study paths, careers and opportunities. Allows basic HTML: h2, h3, p, b, table, tr, td, th. Required; wired via `LLM_REPORT_PROFILE_SYSTEM_PROMPT_FILE`. Process exits if unset or file missing.
- **report_ai_context.txt** – Set `LLM_REPORT_AI_CONTEXT_FILE` in .env. Empty = disabled; set to conf/report_ai_context.txt to append this content to the report profile system prompt. Variable must be present (see .env.example). If the file is missing at the given path, the report runs without this context.
...
# Assessment model: dimensions (aptitudes, traits, values) and skills (interview + recommendations)

Assessment data files live in **src/data/** (not in conf/). Dimension files (used during testing) each define dimensions with a unique dimensionId per element:

- **src/data/dimension_aptitudes.json** – Aptitudes: natural affinity and raw potential (e.g. technical_hands_on, creative_open_ended). Largely stable; dictate ceiling and speed of learning.
- **src/data/dimension_traits.json** – Traits: personality and behavioral tendencies (e.g. experimentation_risk_tolerance, social_collaboration, ambiguity_resilience). How you behave; stable “handling” that shapes fit for environments.
- **src/data/dimension_values.json** – Values: non-negotiables and destination (e.g. helping_others_impact, mastery_growth, autonomy_independence). The fuel that keeps you going; used for career cluster and mission fit.
...
Each dimension file has version, sources, description, and an array of items with id, dimensionId, name, description, how_measured_or_observed, question_hints. Used by the API to generate interview questions and drive assessment. Sources: Citrini 2028 GIC, Microsoft New Future of Work Report 2025.
